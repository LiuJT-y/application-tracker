...
| **简历管理** `/resumes` | 两个 tab:**总表**(把简历拆成可复用条目)+ **简历版本**(从条目组合成不同版本、设默认、看面试率) |
...
每张版本卡片上都能给这一版挂一份 PDF 简历原件:

- **上传**:卡片上点「＋ 上传 PDF」,选一个 `.pdf` 文件即可(单文件 **上限 4MB**)。
- **预览**:上传后点「预览」,在浏览器新标签里直接打开这份 PDF。
- **替换 / 删除**:点「替换」换一份;点「删除」(二次确认)移除。
- 只接受 PDF;选了非 PDF 或超过 4MB 会**当场提示报错**,不会静默失败。

> PDF 字节**直接存在数据库里**(不依赖任何对象存储 / 云盘),所以海外 Neon 部署和大陆本机部署**两种方式都通用、开箱即用**。列表加载时只读文件名和大小,不会把整份 PDF 拉下来,翻得很快。内嵌翻页画廊会在后续版本(3b)加上。
